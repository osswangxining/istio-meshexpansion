...
function istioClusterEnv() {
  ...
  #CIDR=${SERVICE_CIDR:-$(gcloud container clusters describe ${K8S_CLUSTER} ${GCP_OPTS:-} --format "value(servicesIpv4Cidr)")}
  CIDR=`kubectl -n kube-system get cm kubeadm-config  -o yaml | grep serviceSubnet | awk '{print $2}'`
...
function istioBootstrapVM() {
  ...
  DEFAULT_SCRIPT="./setupIstioVM.sh"
  SETUP_ISTIO_VM_SCRIPT=${SETUP_ISTIO_VM_SCRIPT:-${DEFAULT_SCRIPT}}
  echo "Making certs for service account $SA (namespace $NS)"
...
if [[ ${1:-} == "generateClusterEnvAndDnsmasq" ]] ; then
  shift
  istioClusterEnv $1
  istioDnsmasq
